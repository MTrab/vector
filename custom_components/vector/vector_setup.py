...
class VectorSetup:
    ...
    async def async_configure(self) -> None:
        ...
        _LOGGER.debug("Getting cert")
        await self._async_get_cert()
        _LOGGER.debug("Saving cert")
        await self._async_save_cert()
        _LOGGER.debug("Vallidating cert")
        await self._async_validate_cert_name()
        if not self._escapepod:
            _LOGGER.debug("Getting session token")
            await self._async_get_session_token()
        _LOGGER.debug("User authentication")
    ...
    async def _async_get_cert(self) -> bytes:
        ...
        if self._escapepod:
            ...
            sock = context.wrap_socket(conn, server_hostname=self._ip)
            sock.connect((self._ip, 443))
            cert = ssl.DER_cert_to_PEM_cert(sock.getpeercert(True))
            self._cert = str.encode(cert)
        else:
    ...
    async def _async_user_authentication(self) -> str:
        ...
        try:
            ...
            request = messaging.protocol.UserAuthenticationRequest(
                user_session_id=(
                    self._token["session"]["session_token"].encode("utf-8")
                    if not self._escapepod
                    else bytes("Anything1", "UTF-8")
                ),
            ...
            if (
                ...
                raise Exception(
                    ...
                    f"Please be sure to first set up Vector using the companion app. ({response.code})"
                )
