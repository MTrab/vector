...
from ..const import STATE_CARRYING_OBJECT, STATE_HEAD_TRACKING_ID
    ...
    async def on_robot_state(self, robot: Robot, event_type, event, *args, **kwargs):
        ...
        if hasattr(event, "carrying_object_id"):
            if event.carrying_object_id == -1:
                self.coordinator.states.set_robot_attribute(STATE_CARRYING_OBJECT, None)
            else:
