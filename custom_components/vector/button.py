...
from .helpers.speech import Speech, SpeechType
...
class VectorBaseButtonEntity(VectorBase, ButtonEntity):
    ...
    async def _async_press_action(self) -> None:
        ...
        if not isinstance(self.entity_description.command_fn, type(None)):
            await self.entity_description.command_fn(self.coordinator)
        elif not isinstance(self.entity_description.speech_type, type(None)):
            speech: Speech = self.hass.data[DOMAIN][self.entry.entry_id]["speech"]
            await speech.async_speak_predefined(SpeechType.JOKE)
