...
class ImageText:  # pylint: disable=too-few-public-methods
    ...
    def render(self, draw: ImageDraw.ImageDraw, bounds: tuple) -> ImageDraw.ImageDraw:
        ...
        if self.position.value == AnnotationPosition.CENTER_CENTER.value:
            ...
            if (
                self.position.value & AnnotationPosition.LEFT.value
                or self.position.value & AnnotationPosition.RIGHT.value
            ...
            if (
                self.position.value & AnnotationPosition.TOP.value
                or self.position.value & AnnotationPosition.BOTTOM.value
            ) and self.position.value & AnnotationPosition.CENTER.value:
                x = bx1 + (bx2 - bx1 - (text_width)) / 2
